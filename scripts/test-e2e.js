...
writeCodexState(codexA, [
  ...
  ["session-older", "Older e2e session", "Older preview should not win", "Older first message should not win", olderConversationAtMs - 3600000, olderConversationAtMs]
]);
writeJsonl(olderSessionPath, [
  {
    type: "session_meta",
    payload: {
      id: "session-older",
...
const byAgentCodex = JSON.parse(agent(projectB, codexB, claudeB, ["log", "--agent", "codex", "--json"]));
const byAgentClaudeBranch = JSON.parse(agent(projectB, codexB, claudeB, ["log", "--agent=claude", "--branch=main", "--json"]));
...
assert.equal(byAgentCodex.length, 2);
assert.equal(byAgentCodex.every((item) => item.agent === "codex"), true);
assert.equal(byAgentClaudeBranch.length, 1);
assert.equal(byAgentClaudeBranch[0].bundleId, byCurrent[0].bundleId);
...
assert.equal(agent(projectB, codexB, claudeB, ["log", "--title", "Older", "--oneline"]), `1  Older e2e session  ${pushMessage}`);
...
const showAgentClaude = JSON.parse(agent(projectB, codexB, claudeB, ["show", "--agent", "claude", "1", "--json"]));
assert.equal(showAgentClaude.bundleId, byCurrent[0].bundleId);
assert.equal(showAgentClaude.agent, "claude");
...
const restoreCurrentJson = JSON.parse(agent(projectB, codexB, claudeB, ["restore", "--current", "1", "--json"]));
assert.equal(restoreCurrentJson.ok, true);
assert.equal(restoreCurrentJson.results.length, 1);
assert.equal(restoreCurrentJson.results[0].status, "restored");
assert.equal(restoreCurrentJson.results[0].agent, "claude");
assert.equal(restoreCurrentJson.results[0].bundleId, byCurrent[0].bundleId);
assert.equal(typeof restoreCurrentJson.results[0].target, "string");
assert.equal(typeof restoreCurrentJson.results[0].adapted, "boolean");
...
assert.equal(restoreIndexJson.results.length, 1);
assert.equal(restoreIndexJson.results[0].status, "restored");
assert.equal(restoreIndexJson.results[0].agent, "codex");
...
assert.equal(restoreAgentCodexJson.results[0].status, "restored");
assert.equal(restoreAgentCodexJson.results[0].agent, "codex");
assert.equal(restoreAgentCodexJson.results[0].bundleId, byCurrent[1].bundleId);
...
const outOfRangeJson = agentResult(projectB, codexB, claudeB, ["restore", "--index", "4", "--json"]);
assert.notEqual(outOfRangeJson.status, 0);
assert.match(outOfRangeJson.stderr, /restore index 4 is out of range for log \(3 binding\(s\)\)/);
const filteredOutOfRange = agentResult(projectB, codexB, claudeB, ["restore", "--agent", "claude", "2"]);
assert.notEqual(filteredOutOfRange.status, 0);
assert.match(filteredOutOfRange.stderr, /restore index 2 is out of range for agent claude \(1 binding\(s\)\)/);
...
const skippedForeignRestoreJson = JSON.parse(agent(projectB, codexB, claudeB, ["restore", restoreForeignBundle, "--json"]));
assert.equal(skippedForeignRestoreJson.ok, true);
assert.equal(skippedForeignRestoreJson.results.length, 1);
assert.equal(skippedForeignRestoreJson.results[0].status, "skipped");
assert.equal(skippedForeignRestoreJson.results[0].agent, "claude");
assert.equal(skippedForeignRestoreJson.results[0].bundleId, restoreForeignBundle);
assert.equal(skippedForeignRestoreJson.results[0].reason, "claude:foreign-git");
...
function seedForeignCurrentProjectEntry(baseDir, remote, projectId) {
  ...
  writeFileSync(bindingsPath, `${readFileSync(bindingsPath, "utf8")}${JSON.stringify({
    ...
    projectBaseCommit: "current-foreign-commit",
    projectDirty: false,
    bundleId: "claude-current-foreign",
    agent: "claude",
...
function seedForeignStoreProject(baseDir, remote) {
  ...
  writeFileSync(join(projectDir, "bindings.jsonl"), [
    JSON.stringify({
      ...
      projectBaseCommit: "foreign-commit",
      projectDirty: false,
      bundleId: "codex-foreign",
      agent: "codex",
