...
The History view toolbar includes Pull, Push, Sync, Privacy, IR, Clone, Watch, TUI, Refresh, and Clear actions. Pull and Push run the sidecar CLI commands for the current workspace and refresh the history table after completion. Sync shows queue state, Privacy runs a session secret scan, IR inspects a selected bundle as Conversation IR, Clone and Watch cover local Codex `model_provider` sync, and TUI opens `agent-sync tui` in an integrated terminal. The Command Palette also exposes background sync, queue flush, daemon status, repair-local, privacy redaction preview, and readable tool export.
